...
struct Str_Arr*init() {
	struct Str_Arr* My_arr = (struct a*)malloc(sizeof(struct Str_Arr));
...
void listDir(char *startDir, struct Str_Arr* My_arr) {
	...
	while (de) {
		if (de->d_type == DT_DIR && strcmp(de->d_name, ".") != 0 && strcmp(de->d_name, "..") != 0) {			/*копипаста стопроц - норм*/
